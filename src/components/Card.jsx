...
export function Card({ title, description, details, image, author, downloadLink, projectLink, tags = [] }) {
  ...
  // ── Keyboard handler for card trigger (Enter / Space) ──────────────────────
  const handleCardKeyDown = (e) => {
    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openModal(); }
  };

  // ── Accessibility: focus trap + Escape inside modal ────────────────────────
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        {/* Thumbnail — shows skeleton until image loads */}
        {image && (
          <div className="w-full h-48 bg-jade-dark/50 p-6 flex items-center justify-center border-b border-jade-primary/5 overflow-hidden relative">
            ...
            <img
              ...
              className={`max-h-full max-w-full object-contain group-hover:scale-110 transition-transform duration-500 relative z-10 drop-shadow-lg ${imgLoaded ? "opacity-100" : "opacity-0 absolute"}`}
            />
          </div>
        )}
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mt-6">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full bg-jade-dark/50 text-jade-light border border-jade-primary/10 group-hover:border-jade-primary/30 group-hover:text-jade-primary transition-colors"
                >
                  {tag}
                </span>
              ))}
            ...
            >
              ...
              <div className="flex flex-col gap-10 mt-4">
                ...
                <div className="w-full flex flex-col items-center md:items-start">
                  <h3
                    id={`card-title-${title}`}
                    className="text-3xl md:text-4xl font-black text-jade-pale mb-2"
                  >
                    {title}
                  </h3>
                  ...
                  <div className="prose prose-invert max-w-none text-center md:text-left">
                    <p className="text-jade-light leading-loose text-base">{details}</p>
                  </div>

                  {author && (
                    <div className="mt-10 pt-6 border-t border-jade-primary/10 w-full text-center md:text-left">
                      <p className="text-sm text-jade-light/70 font-medium italic">{author}</p>
                    </div>
                  )}

                  <div className="mt-12 flex flex-wrap gap-4 w-full justify-center md:justify-start">
                    {/* View Project link — shown when projectLink is set */}
                    {projectLink && (
                      <a
                        href={projectLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-8 py-4 bg-jade-primary text-jade-dark font-black rounded-xl hover:bg-jade-pale hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all active:scale-95 text-xs uppercase tracking-widest"
                    ...
                    {/* Download link — shown only when downloadLink is a real URL */}
                    {downloadLink && downloadLink !== "#" && (
                      <a
                        href={downloadLink}
                        download
                        className="px-8 py-4 bg-jade-dark border border-jade-primary/20 text-jade-pale font-black rounded-xl hover:bg-jade-deep transition-all active:scale-95 text-xs uppercase tracking-widest"
                        aria-label={`Download ${title} project file`}
                      >
                        Download
                      </a>
                    )}
