                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center relative z-10">
                  ...
                  <div className="lg:col-span-7 flex flex-col justify-between h-full">
                    <div>
                      ...
                      {/* Key Highlights Bento (6 items in 2 columns) */}
                    ...
                    {/* Actions Hub - Explore NotesPack + Showcase link */}
                  ...
                  {/* Right Column: Phone Mockup with Interactive Feature Switcher */}
                  <div className="lg:col-span-5 flex flex-col justify-center items-center">
                    ...
                    >
                      ...
                      {/* Image Container inside Phone Frame with Animated Transitions */}
                      <div className="w-full h-full rounded-[2.35rem] bg-gradient-to-b from-[#070e1b] to-[#020617] overflow-hidden relative flex items-center justify-center border border-jade-primary/10">
                        {!imgError ? (
                          <AnimatePresence mode="wait">
                            <motion.img
                              key={currentScreen.id || activeScreenIndex}
                              src={currentScreen.image || app.previewImage}
                              alt={`${app.title} - ${currentScreen.name}`}
                    ...
                    {screens.length > 0 && (
                      <div className="mt-5 flex flex-col items-center max-w-sm w-full">
                        <div className="flex flex-wrap justify-center gap-1.5 p-1.5 rounded-2xl bg-jade-deep/40 border border-jade-primary/20 backdrop-blur-xl">
                          {screens.map((scr, idx) => (
                            <button
                              key={scr.id}
                              onClick={() => setActiveScreenIndex(idx)}
                              className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-wider transition-all duration-200 cursor-pointer ${
                                activeScreenIndex === idx
                                  ? "bg-jade-primary text-jade-dark shadow-md shadow-jade-primary/25 scale-105"
                                  : "text-jade-light/70 hover:text-jade-pale hover:bg-jade-deep/70"
                              }`}
                              aria-label={`Show ${scr.name} screenshot`}
                            >
                              {scr.name}
                            </button>
                          ))}
                        </div>
                        <p className="text-[11px] text-jade-light/70 text-center mt-2.5 font-medium">
                          {currentScreen.desc}
                        </p>
                      </div>
                    )}
